...
HOME_DIR=/root/
PY3LOCALPATH=`${HOME_DIR}/py3localpath.py`
...
find /usr/lib /usr/local $HOME_DIR/.local | sort > $HOME_DIR/usr-local-1-before-protobuf.txt
...
find /usr/lib /usr/local $HOME_DIR/.local | sort > $HOME_DIR/usr-local-2-after-protobuf.txt
...
# grpc.
find /usr/lib /usr/local $HOME_DIR/.local | sort > $HOME_DIR/usr-local-2b-before-grpc-pip3.txt
pip3 list | tee $HOME_DIR/pip3-list-2b-before-grpc-pip3.txt
sudo pip3 install -rrequirements.txt
GRPC_PYTHON_BUILD_WITH_CYTHON=1 sudo pip3 install .
sudo ldconfig
cd ..

find /usr/lib /usr/local $HOME_DIR/.local | sort > $HOME_DIR/usr-local-3-after-grpc.txt
...
find /usr/lib /usr/local $HOME_DIR/.local | sort > $HOME_DIR/usr-local-4-after-PI.txt
...
find /usr/lib /usr/local $HOME_DIR/.local | sort > $HOME_DIR/usr-local-5-after-behavioral-model.txt

# --- Mininet --- #
git clone --depth=1 https://github.com/mininet/mininet.git
# Try without installing openvswitch, i.e. no '-v' option, to see if everything still works well without it.
./mininet/util/install.sh -nw
# cd mininet
